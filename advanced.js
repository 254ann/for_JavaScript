...
const fetchItems = async () => {
  try {
    const allItems = await getItems(shop);
    return allItems;
  } catch (error) {
    return error;
  }
...
const myShop = async () => {
  const products = await fetchItems();
    products.map((product) => {
      ...
      const shopImages = document.createElement("img")
      shopImages.src = `${product.imageUrl}`
      contentSection.appendChild(shopImages)

      const imageTitle = document.createElement('h6')
      imageTitle.textContent=`${product.title}`
      contentSection.appendChild(imageTitle)
      ...
      shopLocation.textContent=`${product.location}`
      contentSection.appendChild(shopLocation)

      const productPrice = document.createElement('p')
      productPrice.textContent=`ksh ${product.price}`
      contentSection.appendChild(productPrice)
